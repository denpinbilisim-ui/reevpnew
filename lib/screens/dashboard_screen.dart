...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  Widget _buildRatingProductCard(Map<String, dynamic> product) {
    ...
    // Debug: Print product rating info
    print('Product: ${product['name']}, ID: ${product['id']}, Redemption ID: ${product['redemption_id']}, Rating: ${product['user_rating']}');
            ...
            child: Column(
              ...
              children: [
                ...
                if (product['purchase_date'] != null) ...[
                  const SizedBox(height: 4),
                  Text(
                    'Satın alma: ${_formatDate(product['purchase_date'])}',
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
  ...
  Future<void> _submitRating(Map<String, dynamic> product, int rating, String comment) async {
    try {
      ...
      // Use redemption_id or purchase_id to uniquely identify each purchase
      final uniqueId = product['redemption_id']?.toString() ?? 
                      product['purchase_id']?.toString() ?? 
                      '${product['id']}_${product['purchase_date'] ?? DateTime.now().millisecondsSinceEpoch}';
      ...
      if (mounted) {
        ...
        if (response['success'] == true) {
          ...
          // Reload purchase history to get updated ratings
          _loadPurchaseHistory();
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
