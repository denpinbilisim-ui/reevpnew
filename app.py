...
class User(UserMixin, db.Model):
    ...
    customer_qrs = db.relationship('CustomerQR', backref='customer', lazy=True, overlaps="customer_qr_codes")
...
class Branch(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=get_turkey_time)
    scanned_qrs = db.relationship('CustomerQR', backref='scanning_branch', lazy=True, overlaps="scanned_qr_codes")
...
class CustomerQRCode(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('customer_qr_codes', overlaps="customer_qrs,customer"), overlaps="customer,customer_qrs")
    used_by_branch = db.relationship('Branch', backref=db.backref('scanned_qr_codes', overlaps="scanned_qrs,scanning_branch"), overlaps="scanned_qrs,scanning_branch")
...
class Survey(db.Model):
    ...
    end_date = db.Column(db.DateTime, nullable=False)
    is_active = db.Column(db.Boolean, default=True)
    reward_points = db.Column(db.Integer, default=0)  # Anket tamamlandığında verilecek puan
    created_at = db.Column(db.DateTime, default=get_turkey_time)
    updated_at = db.Column(db.DateTime, default=get_turkey_time, onupdate=get_turkey_time)
...
# Kullanıcının anket görüntüleme takibi
class UserSurveyView(db.Model):
    __tablename__ = 'user_survey_view'
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    survey_id = db.Column(db.Integer, db.ForeignKey('survey.id'), nullable=False)
    viewed_at = db.Column(db.DateTime, default=get_turkey_time)
    
    # Relationships
    user = db.relationship('User', backref='survey_views')
    survey = db.relationship('Survey', backref='user_views')
    
    # Unique constraint - bir kullanıcı bir anketi sadece bir kez görüntüleyebilir
    __table_args__ = (db.UniqueConstraint('user_id', 'survey_id', name='unique_user_survey_view'),)
...
def inject_survey_notifications():
    """Tüm template'lere anket bildirim sayısını ekle"""
    if current_user.is_authenticated and not current_user.is_admin:
        unviewed_count = get_unviewed_surveys_count(current_user.id)
        return {'unviewed_surveys_count': unviewed_count}
    return {'unviewed_surveys_count': 0}
...
def get_unviewed_surveys_count(user_id):
    """Kullanıcının görülmemiş anket sayısını döndür"""
    try:
        # Kullanıcının daha önce cevapladığı anketleri al
        completed_survey_ids = [r.survey_id for r in SurveyResponse.query.filter_by(user_id=user_id).all()]
        
        # Kullanıcının görüntülediği anketleri al
        viewed_survey_ids = [v.survey_id for v in UserSurveyView.query.filter_by(user_id=user_id).all()]
        
        # Aktif ve geçerli anketleri al
        now = get_turkey_time().replace(tzinfo=None)
        active_surveys = Survey.query.filter(
            Survey.is_active == True,
            Survey.start_date <= now,
            Survey.end_date >= now,
            ~Survey.id.in_(completed_survey_ids)  # Tamamlanmamış anketler
        ).all()
        
        # Görülmemiş anket sayısını hesapla
        unviewed_count = 0
        for survey in active_surveys:
            if survey.id not in viewed_survey_ids:
                unviewed_count += 1
        
        return unviewed_count
    except Exception as e:
        print(f"Error getting unviewed surveys count: {e}")
        return 0
...
@app.context_processor
def inject_user():
    # Site logosu ve splash resmi için global değişkenler
    site_logo = SiteSetting.query.filter_by(key='site_logo').first()
    splash_image = SiteSetting.query.filter_by(key='splash_image').first()
    return dict(current_user=current_user, site_logo=site_logo, splash_image=splash_image)
...
def dashboard():
    ...
    # En beğenilen ürünü getir
    most_liked_product = get_most_liked_product(current_user.id)
    
    # Splash resmini getir
    splash_image = SiteSetting.query.filter_by(key='splash_image').first()
...
def purchase_history():
    ...
    # Puan kazanımları
    points_query = CustomerQRCode.query.filter_by(customer_id=current_user.id, is_used=True)
    if start_date:
        try:
            start_dt = datetime.strptime(start_date, '%Y-%m-%d')
            points_query = points_query.filter(CustomerQRCode.used_at >= start_dt)
        except ValueError:
            pass
    if end_date:
        try:
            end_dt = datetime.strptime(end_date, '%Y-%m-%d')
            end_dt = end_dt.replace(hour=23, minute=59, second=59)
            points_query = points_query.filter(CustomerQRCode.used_at <= end_dt)
        except ValueError:
            pass
    ...
    total_points_earned = CustomerQRCode.query.filter_by(customer_id=current_user.id, is_used=True).count()
...
# En çok satın alınan ürünü getir (en beğenilen ürün olarak göster)
def get_most_liked_product(user_id):
    """Kullanıcının en çok satın aldığı ürünü döndürür"""
    try:
        # En çok satın alınan ürünü bul (ProductRedemption tablosundan)
        from sqlalchemy import func
        most_purchased = db.session.query(
            ...
            Product.id, Product.name, Product.description, Product.image_filename, Product.points_required
        ).order_by(
        ...
        if most_purchased:
            return {
                'id': most_purchased.id,
                'name': most_purchased.name,
                'description': most_purchased.description,
                'purchase_count': most_purchased.purchase_count,
                'points_required': most_purchased.points_required,
                'image_filename': most_purchased.image_filename
            }
        else:
            # Eğer onaylanmış satın alma yoksa, tüm satın almalara bak
            any_purchased = db.session.query(
                Product.id,
                Product.name,
                Product.description,
                Product.image_filename,
            ...
            if any_purchased:
                return {
                    'id': any_purchased.id,
                    'name': any_purchased.name,
                    'description': any_purchased.description,
                    'purchase_count': any_purchased.purchase_count,
                    'points_required': any_purchased.points_required,
                    'image_filename': any_purchased.image_filename
                }
        
        return None
    except Exception as e:
        print(f"Error getting most purchased product: {e}")
        return None
...
def profile():
    ...
    # En beğenilen ürünü getir
    most_liked_product = get_most_liked_product(current_user.id)
    return render_template('profile.html', purchase_count=purchase_count, branches=branches, most_liked_product=most_liked_product)
...
def api_dashboard():
    try:
        ...
        # En beğenilen ürünü getir (rating tablosundan)
        most_liked_product = get_most_liked_product(user.id)
...
# Hesap Silme API - Kullanıcı kendi hesabını ve ilişkili tüm verilerini kalıcı olarak siler
...
def api_delete_account():
    try:
        ...
        password = data.get('password')
        if not password:
            return jsonify({'success': False, 'error': 'Şifre onayı gerekli'}), 400

        if not user.check_password(password):
            return jsonify({'success': False, 'error': 'Şifre yanlış'}), 400

        user_id = user.id

        # Cascade ilişkisi olan kayıtlar - ORM üzerinden silinmeli (SurveyAnswer'lar otomatik silinir)
        survey_responses = SurveyResponse.query.filter_by(user_id=user_id).all()
        for survey_response in survey_responses:
            db.session.delete(survey_response)

        # Bağımsız ilişkili kayıtlar
        UserSurveyView.query.filter_by(user_id=user_id).delete(synchronize_session=False)
        ...
        ProductRating.query.filter_by(user_id=user_id).delete(synchronize_session=False)
        ProductRedemption.query.filter_by(user_id=user_id).delete(synchronize_session=False)
        CampaignUsage.query.filter_by(customer_id=user_id).delete(synchronize_session=False)
        CustomerQRCode.query.filter_by(customer_id=user_id).delete(synchronize_session=False)
...
def api_purchase_history():
    try:
        ...
        for redemption, product, category in redemptions:
            # Kullanıcının bu spesifik satın alma için verdiği puanı kontrol et
            # 1. Önce sanal product_id ile kontrol et (yeni sistem)
            virtual_product_id = int(redemption.product_id) * 1000000 + int(redemption.id)
            rating = ProductRating.query.filter_by(
                user_id=user_id,
                product_id=virtual_product_id
            ).first()
            
            # 2. Sanal product_id ile bulunamazsa, comment içinde redemption_id ara
            if not rating:
                rating = ProductRating.query.filter_by(
                    user_id=user_id,
                    product_id=redemption.product_id
                ).filter(ProductRating.comment.like(f'%redemption_id:{redemption.id}%')).first()
            
            # 3. Hala bulunamazsa, eski yöntemle kontrol et (geriye uyumluluk)
            if not rating:
                rating = ProductRating.query.filter_by(
                    user_id=user_id,
                    product_id=redemption.product_id
                ).filter(~ProductRating.comment.like('%redemption_id:%')).first()
            
            # Debug bilgisi
            print(f"Redemption ID: {redemption.id}, Product: {product.name if product else 'Unknown'}, Rating: {rating.rating if rating else 'None'}")
...
def api_rate_product():
    try:
        ...
        redemption_id = data.get('redemption_id')  # Benzersiz satın alma kimliği
        ...
        # Mevcut puanı kontrol et - redemption_id varsa ona göre, yoksa eski yöntemle
        if redemption_id:
            # 1. Önce sanal product_id ile kontrol et (yeni sistem)
            virtual_product_id = int(product_id) * 1000000 + int(redemption_id)
            ...
            if not existing_rating:
                existing_rating = ProductRating.query.filter_by(
                    user_id=user_id,
                    product_id=product_id
                ).filter(ProductRating.comment.like(f'%redemption_id:{redemption_id}%')).first()
            
            # 3. Hala bulunamazsa, genel puanı kontrol et
            if not existing_rating:
                general_rating = ProductRating.query.filter_by(
                    user_id=user_id,
                    product_id=product_id
                ).filter(~ProductRating.comment.like('%redemption_id:%')).first()
                
                if general_rating:
                    # Genel puanı bu redemption_id için özelleştir (sanal product_id ile yeni kayıt oluştur)
                    new_rating = ProductRating(
                        user_id=user_id,
                        product_id=virtual_product_id,
                        rating=rating,
                        comment=f"{comment}\n[redemption_id:{redemption_id}]" if comment else f"[redemption_id:{redemption_id}]",
                        created_at=get_turkey_time()
                    )
                    db.session.add(new_rating)
                    existing_rating = new_rating
        else:
            # Eski yöntem - sadece user_id + product_id
            existing_rating = ProductRating.query.filter_by(
                user_id=user_id,
                product_id=product_id
            ).first()
        ...
        if existing_rating:
            ...
            if redemption_id:
                # redemption_id'yi comment'e ekle veya güncelle
                current_comment = existing_rating.comment or ''
                if f'redemption_id:{redemption_id}' not in current_comment:
                    existing_rating.comment = f"{comment}\n[redemption_id:{redemption_id}]" if comment else f"[redemption_id:{redemption_id}]"
                else:
                    # Mevcut redemption_id'li comment'i güncelle
                    lines = current_comment.split('\n')
                    new_lines = []
                    redemption_line_found = False
                    for line in lines:
                        if f'redemption_id:{redemption_id}' in line:
                            redemption_line_found = True
                            continue  # Bu satırı atla
                        elif not line.startswith('[redemption_id:'):
                            new_lines.append(line)
                    
                    if comment:
                        new_lines.insert(0, comment)
                    new_lines.append(f'[redemption_id:{redemption_id}]')
                    existing_rating.comment = '\n'.join(new_lines).strip()
            else:
                existing_rating.comment = comment
            existing_rating.updated_at = get_turkey_time()
        else:
            # Yeni puan oluştur - sadece redemption_id yoksa
            if not redemption_id:
                ...
                # redemption_id varsa ama puan yoksa, sanal bir product_id kullan
                # UNIQUE constraint'i bypass etmek için product_id'yi modifiye et
                # Format: original_product_id * 1000000 + redemption_id
                # Bu sayede her redemption_id için benzersiz bir product_id elde ederiz
                virtual_product_id = int(product_id) * 1000000 + int(redemption_id)
                
                final_comment = f"{comment}\n[redemption_id:{redemption_id}]" if comment else f"[redemption_id:{redemption_id}]"
                new_rating = ProductRating(
                    user_id=user_id,
                    product_id=virtual_product_id,  # Sanal product_id kullan
                    rating=rating,
                    comment=final_comment,
                    created_at=get_turkey_time()
                )
...
# Admin Rating Report API
@app.route('/admin/rating-report', methods=['GET'])
@login_required
def admin_rating_report():
    ...
    try:
        ...
        query = db.session.query(
            ...
            User.name.label('user_name'),
            Product.name.label('product_name')
        ).join(User, ProductRating.user_id == User.id).outerjoin(
            Product, ProductRating.product_id == Product.id
        )
        
        # Apply filters
        if start_date:
            query = query.filter(ProductRating.created_at >= start_date)
        if end_date:
            query = query.filter(ProductRating.created_at <= end_date + ' 23:59:59')
        if min_rating:
            query = query.filter(ProductRating.rating >= int(min_rating))
        
        # Execute query
        results = query.order_by(ProductRating.created_at.desc()).all()
        
        # Process results
        ratings = []
        for rating, user_name, product_name in results:
            # Extract redemption_id from comment if exists
            redemption_id = None
            if rating.comment and 'redemption_id:' in rating.comment:
                import re
                match = re.search(r'redemption_id:(\d+)', rating.comment)
                if match:
                    redemption_id = match.group(1)
            
            # Clean comment (remove redemption_id tags)
            clean_comment = rating.comment
            if clean_comment and '[redemption_id:' in clean_comment:
                clean_comment = re.sub(r'\[redemption_id:\d+\]', '', clean_comment).strip()
                if clean_comment.endswith('\n'):
                    clean_comment = clean_comment[:-1]
            
            # Handle virtual product IDs (convert back to original)
            original_product_id = rating.product_id
            if rating.product_id > 1000000:
                original_product_id = rating.product_id // 1000000
                if not redemption_id:
                    redemption_id = str(rating.product_id % 1000000)
            
            # Get actual product name if it's a virtual product ID
            if rating.product_id > 1000000 and not product_name:
                actual_product = Product.query.get(original_product_id)
                product_name = actual_product.name if actual_product else f'Ürün #{original_product_id}'
            
            ratings.append({
                'user_name': user_name,
                'product_name': product_name or f'Ürün #{original_product_id}',
        ...
        unique_products = len(set(r['product_name'] for r in ratings))
        unique_users = len(set(r['user_name'] for r in ratings))
        ...
        print(f"Rating report error: {str(e)}")
        return jsonify({'success': False, 'error': f'Hata: {str(e)}'}), 500
...
@login_required
def test_rate():
    return f"<h1>Test Sayfası</h1><p>Kullanıcı: {current_user.name}</p><p>ID: {current_user.id}</p>"

# Ürün Puanlama Routes
@app.route('/rate-products')
@login_required
def rate_products():
    """Kullanıcının aldığı ürünleri puanlama sayfası"""
    print(f"Rate products accessed by user: {current_user.id} - {current_user.name}")
    
    try:
        # Önce kullanıcının tüm ProductRedemption kayıtlarını kontrol et
        all_redemptions = ProductRedemption.query.filter_by(user_id=current_user.id).all()
        print(f"Total redemptions for user {current_user.id}: {len(all_redemptions)}")
        
        for redemption in all_redemptions:
            print(f"Redemption ID: {redemption.id}, Product ID: {redemption.product_id}, Confirmed: {redemption.is_confirmed}")
        
        # Kullanıcının aldığı ürünleri getir - her satın alma ayrı ayrı
        user_purchases = db.session.query(
            ProductRedemption.id.label('redemption_id'),
            ProductRedemption.product_id,
            ProductRedemption.redeemed_at,
            ProductRedemption.points_used,
            Product.name,
            Product.description,
            Product.image_filename.label('image'),
            Product.points_required
        ).join(
            Product, ProductRedemption.product_id == Product.id
        ).filter(
            ProductRedemption.user_id == current_user.id
        ).order_by(
            ProductRedemption.redeemed_at.desc()
        ).all()
        
        print(f"Found {len(user_purchases)} individual purchases for user {current_user.id}")
        
        products_with_ratings = []
        for purchase in user_purchases:
            # Her satın alma için ayrı puanlama kontrol et
            existing_rating = ProductRating.query.filter_by(
                user_id=current_user.id,
                product_id=purchase.product_id
            ).filter(
                ProductRating.comment.like(f'%[redemption_id:{purchase.redemption_id}]%')
            ).first()
            
            # Eğer redemption_id bazlı puanlama yoksa, eski genel puanlamayı kontrol et
            if not existing_rating:
                existing_rating = ProductRating.query.filter_by(
                    user_id=current_user.id,
                    product_id=purchase.product_id
                ).filter(
                    ~ProductRating.comment.like('%[redemption_id:%')
                ).first()
            
            products_with_ratings.append({
                'redemption_id': purchase.redemption_id,
                'product_id': purchase.product_id,
                'name': purchase.name,
                'description': purchase.description,
                'image': purchase.image,
                'points_required': purchase.points_required,
                'points_used': purchase.points_used,
                'purchase_date': purchase.redeemed_at,
                'current_rating': existing_rating.rating if existing_rating else None,
                'current_comment': existing_rating.comment if existing_rating else None,
                'rating_date': existing_rating.created_at if existing_rating else None
            })
        
        return render_template('rate_products.html', products=products_with_ratings)
        
    except Exception as e:
        print(f"Rate products error: {str(e)}")
        import traceback
        traceback.print_exc()
...
@login_required
def submit_rating():
    """Ürün puanlaması gönder"""
    try:
        print("=== SUBMIT RATING DEBUG ===")
        print("Form data:", dict(request.form))
        print("User:", current_user.id, current_user.name)
        
        product_id = int(request.form.get('product_id'))
        redemption_id = int(request.form.get('redemption_id'))
        print(f"Product ID: {product_id}, Redemption ID: {redemption_id}")
        
        # Rating field name artık product_id ve redemption_id ile birlikte
        rating_field_name = f'rating_{product_id}_{redemption_id}'
        print(f"Looking for rating field: {rating_field_name}")
        
        rating_value = request.form.get(rating_field_name)
        print(f"Rating value from form: {rating_value}")
        
        if not rating_value:
            print("ERROR: Rating field not found!")
            flash('Puan seçimi bulunamadı. Lütfen tekrar deneyin.', 'error')
            return redirect('/rate-products')
            
        rating = int(rating_value)
        comment = request.form.get('comment', '').strip()
        print(f"Final rating: {rating}, comment length: {len(comment)}")
        
        if not (1 <= rating <= 5):
            flash('Puan 1-5 arasında olmalıdır.', 'error')
            return redirect('/rate-products')
        
        # Kullanıcının bu satın alma işlemini yaptığını kontrol et
        purchase_exists = ProductRedemption.query.filter_by(
            id=redemption_id,
            user_id=current_user.id,
            product_id=product_id
        ).first()
        ...
        if not purchase_exists:
            flash('Sadece aldığınız ürünleri puanlayabilirsiniz.', 'error')
            return redirect('/rate-products')
        
        # Bu satın alma için mevcut puanlamayı kontrol et
        existing_rating = ProductRating.query.filter_by(
            user_id=current_user.id,
            product_id=product_id
        ).filter(
            ProductRating.comment.like(f'%[redemption_id:{redemption_id}]%')
        ).first()
        
        if existing_rating:
            print("ERROR: Bu satın alma zaten puanlanmış!")
            flash('Bu ürünü zaten puanladınız. Her satın alma için sadece bir kez puan verilebilir.', 'error')
            return redirect('/rate-products')
        
        # Yoruma redemption_id ekle
        if comment:
            comment_with_id = f"{comment}\n[redemption_id:{redemption_id}]"
        else:
            comment_with_id = f"[redemption_id:{redemption_id}]"
        ...
        new_rating = ProductRating(
            ...
            product_id=product_id,
            rating=rating,
            comment=comment_with_id,
            created_at=get_turkey_time()
        )
        db.session.add(new_rating)
        message = 'Puanlamanız kaydedildi! Teşekkürler.'
        print(f"New rating created: User {current_user.id}, Product {product_id}, Rating {rating}")
        ...
        db.session.rollback()
        flash(f'Puanlama kaydedilirken hata oluştu: {str(e)}', 'error')
...
def surveys():
    ...
    try:
        # Kullanıcının daha önce cevapladığı anketleri al
        completed_survey_ids = [r.survey_id for r in SurveyResponse.query.filter_by(user_id=current_user.id).all()]
        
        # Aktif ve geçerli anketleri al (kullanıcının cevaplamadığı)
        active_surveys = Survey.query.filter(
            Survey.is_active == True,
            ~Survey.id.in_(completed_survey_ids)
        ).all()
        
        # Geçerli tarihteki anketleri filtrele
        valid_surveys = [survey for survey in active_surveys if survey.is_valid()]
        
        # Kullanıcının bu anketleri görüntülediğini kaydet
        for survey in valid_surveys:
            existing_view = UserSurveyView.query.filter_by(
                user_id=current_user.id,
                survey_id=survey.id
            ).first()
            
            if not existing_view:
                view_record = UserSurveyView(
                    user_id=current_user.id,
                    survey_id=survey.id,
                    viewed_at=get_turkey_time()
                )
                db.session.add(view_record)
        
        db.session.commit()
        
        return render_template('surveys.html', surveys=valid_surveys)
    except Exception as e:
        db.session.rollback()
        flash(f'Anketler yüklenirken hata oluştu: {str(e)}', 'error')
        return redirect(url_for('dashboard'))
...
def take_survey(survey_id):
    ...
    try:
        ...
        # Anket geçerli mi kontrol et
        if not survey.is_valid():
            flash('Bu anket artık aktif değil.', 'error')
        ...
        if existing_response:
            flash('Bu anketi daha önce cevapladınız.', 'info')
            return redirect(url_for('surveys'))
        
        # Anket sorularını sıraya göre getir
        questions = SurveyQuestion.query.filter_by(survey_id=survey_id).order_by(SurveyQuestion.question_order).all()
        
        return render_template('take_survey.html', survey=survey, questions=questions)
    except Exception as e:
        flash(f'Anket yüklenirken hata oluştu: {str(e)}', 'error')
        return redirect(url_for('surveys'))

@app.route('/submit_survey/<int:survey_id>', methods=['POST'])
@login_required
def submit_survey(survey_id):
    """Anket cevaplarını kaydet ve puan ver"""
    try:
        survey = Survey.query.get_or_404(survey_id)
        ...
        for question in questions:
            ...
            if answer_key in request.form:
                ...
                if question.question_type == 'rating':
                    answer.answer_rating = int(request.form[answer_key])
                elif question.question_type == 'yes_no':
                    answer.answer_boolean = request.form[answer_key].lower() == 'true'
                elif question.question_type == 'multiple_choice':
                    answer.answer_choice = request.form[answer_key]
                else:  # text
                    answer.answer_text = request.form[answer_key]
                
                db.session.add(answer)
        
        # Kullanıcıya puan ver
        if survey.reward_points > 0:
            current_user.points += survey.reward_points
            ...
            transaction = Transaction(
                ...
                description=f'Anket tamamlama ödülü: {survey.title}',
                timestamp=get_turkey_time()
            )
            db.session.add(transaction)
        ...
        if survey.reward_points > 0:
            flash(f'Anket tamamlandı! {survey.reward_points} puan kazandınız.', 'success')
        else:
            flash('Anket başarıyla tamamlandı. Teşekkürler!', 'success')
        
        return redirect(url_for('surveys'))
        
    except Exception as e:
        db.session.rollback()
        flash(f'Anket kaydedilirken hata oluştu: {str(e)}', 'error')
...
# Admin Survey API Routes
@app.route('/admin/get_surveys', methods=['GET'])
@login_required
def get_surveys():
    """Anketleri JSON olarak döndür"""
    if not current_user.is_admin:
        return jsonify({'success': False, 'error': 'Yetkisiz erişim'}), 403
...
@app.route('/admin/survey/<int:survey_id>/details', methods=['GET'])
@login_required
def survey_details(survey_id):
    """Anket detaylarını döndür"""
    if not current_user.is_admin:
        return jsonify({'success': False, 'error': 'Yetkisiz erişim'}), 403
    ...
    try:
        ...
        return jsonify({
            ...
            'survey': {
                ...
                'title': survey.title,
                'description': survey.description,
                'start_date': survey.start_date.strftime('%d.%m.%Y %H:%M') if survey.start_date else None,
                'end_date': survey.end_date.strftime('%d.%m.%Y %H:%M') if survey.end_date else None,
                'reward_points': survey.reward_points,
                'is_active': survey.is_active
            },
...
def toggle_survey(survey_id):
    ...
    try:
        ...
        survey.updated_at = get_turkey_time()
        db.session.commit()
        
        status = "aktif" if survey.is_active else "pasif"
        return jsonify({
            'success': True,
            'message': f'Anket {status} duruma getirildi.',
            'is_active': survey.is_active
        })
...
@app.route('/admin/survey/<int:survey_id>/edit', methods=['PUT'])
@login_required
def edit_survey(survey_id):
    ...
    try:
        ...
        survey.reward_points = data.get('reward_points', survey.reward_points)
        survey.is_active = data.get('is_active', survey.is_active)
        
        # Tarihleri güncelle
        if data.get('start_date'):
            survey.start_date = datetime.strptime(data['start_date'], '%Y-%m-%d').date()
        if data.get('end_date'):
            survey.end_date = datetime.strptime(data['end_date'], '%Y-%m-%d').date()
...
@app.route('/admin/survey/<int:survey_id>/delete', methods=['DELETE'])
@login_required
def delete_survey(survey_id):
    ...
    try:
        ...
        # İlişkili kayıtları sil (cascade ile otomatik silinecek)
